...
public final class BaseDonneeH2 implements IBaseDonnee {
    ...
    private void createSchema() {
        String query = new StringBuilder().append("CREATE SCHEMA IF NOT EXISTS IHMProjetFF;")
                ...
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.Utilisateur (idUtilisateur INT NOT NULL AUTO_INCREMENT, idEntite INT NOT NULL, numeroEtudiant INT, nomUtilisateur VARCHAR2(50), prenomUtilisateur VARCHAR2(50), identifiantUtilisateur VARCHAR2(50), motDePasseUtilisateur VARCHAR2(250), PRIMARY KEY(idUtilisateur), FOREIGN KEY(idEntite) REFERENCES IHMProjetFF.ENTITE(idEntite));")
                        //La table Question contient toutes les questions disponibles pour l'ajout dans un QCM.
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.Question (idQuestion INT NOT NULL AUTO_INCREMENT, intituleQuestion VARCHAR2(250), multipleQuestion BOOLEAN, dureeQuestion INT NOT NULL, pointQuestion INT NOT NULL, PRIMARY KEY(idQuestion));")
                        //La table Reponse contient toutes les réponses associées à une question donnée.
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.Reponse (idReponse INT NOT NULL AUTO_INCREMENT, idQuestion INT NOT NULL, intituleReponse VARCHAR2(250), correctReponse BOOLEAN, PRIMARY KEY(idReponse), FOREIGN KEY(idQuestion) REFERENCES IHMProjetFF.Question(idQuestion) ON DELETE CASCADE);")
                        //La table QCM contient tout les QCM créés dans l'aplication.
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.QCM (idQCM INT NOT NULL AUTO_INCREMENT, idCreateur INT NOT NULL, nomQCM VARCHAR2(250), dateCreation DATE, PRIMARY KEY(idQCM), etatQCM VARCHAR2(250), FOREIGN KEY(idCreateur) REFERENCES IHMProjetFF.Utilisateur(idUtilisateur) ON DELETE CASCADE);")
                        //La table QCMQuestion contient toutes les questions associées à un QCM donné.
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.QCMQuestion (idQCM INT NOT NULL, idQuestion INT NOT NULL, PRIMARY KEY(idQCM, idQuestion), FOREIGN KEY(idQCM) REFERENCES IHMProjetFF.QCM(idQCM) ON DELETE CASCADE, FOREIGN KEY(idQuestion) REFERENCES IHMProjetFF.Question(idQuestion) ON DELETE CASCADE);")
                        //La table ResultatUtilisateur contient toutes les participations à un QCM pour un utilisateur donné.
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.ResultatUtilisateur (idResultatUtilisateur INT NOT NULL AUTO_INCREMENT, idUtilisateur INT NOT NULL, idQCM INT NOT NULL, dateResultatUtilisateur DATE, score INT, PRIMARY KEY(idResultatUtilisateur), FOREIGN KEY(idQCM) REFERENCES IHMProjetFF.QCM(idQCM) ON DELETE CASCADE, FOREIGN KEY(idUtilisateur) REFERENCES IHMProjetFF.Utilisateur(idUtilisateur) ON DELETE CASCADE);")
                ...
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.ReponseUtilisateur (idResultatUtilisateur INT NOT NULL, idReponse INT NOT NULL, PRIMARY KEY(idResultatUtilisateur, idReponse), FOREIGN KEY(idResultatUtilisateur) REFERENCES IHMProjetFF.ResultatUtilisateur(idResultatUtilisateur) ON DELETE CASCADE, FOREIGN KEY(idReponse) REFERENCES IHMProjetFF.Reponse(idReponse) ON DELETE CASCADE);")
                ...
                .append("CREATE TABLE IF NOT EXISTS IHMProjetFF.MemoGetNextQuestion (idResultatUtilisateur INT NOT NULL, idQuestion INT NOT NULL, PRIMARY KEY(idResultatUtilisateur, idQuestion), FOREIGN KEY(idResultatUtilisateur) REFERENCES IHMProjetFF.ResultatUtilisateur(idResultatUtilisateur) ON DELETE CASCADE, FOREIGN KEY(idQuestion) REFERENCES IHMProjetFF.Question(idQuestion) ON DELETE CASCADE);").toString();
